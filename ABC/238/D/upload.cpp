...
using namespace std;
using namespace lib;
bool check(u64 s, u64 x, u64 y, u64 b) {
  static set<tuple<u64, u64, u64, u64>> check_list;
  ...
  if (diff) {
    return check(s, x | checker, y, b + 1);
  } else {
    return check(s, x, y, b + 1) or check(s, x | checker, y | checker, b + 1);
  }
}
int main() {
  u64 T = read<u64>();
  for (u64 i = 0; i < T; ++i) {
    ...
    if (check(s, a, a, 0)) cout << "Yes" << endl;
    else
      cout << "No" << endl;
  }
  return 0;
